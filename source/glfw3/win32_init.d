...
//
extern(Windows) BOOL DllMain(HINSTANCE instance, DWORD reason, LPVOID reserved) {
    return TRUE;
}
...
private GLFWbool loadLibraries() {
    ...
    _glfw.win32.winmm.GetTime = cast(PFN_timeGetTime)
    ...
    _glfw.win32.user32.SetProcessDPIAware_ = cast(PFN_SetProcessDPIAware)
    ...
    _glfw.win32.user32.SetProcessDpiAwarenessContext_ = cast(PFN_SetProcessDpiAwarenessContext)
        GetProcAddress(_glfw.win32.user32.instance, "SetProcessDpiAwarenessContext");
    _glfw.win32.user32.GetDpiForWindow_ = cast(PFN_GetDpiForWindow)
        GetProcAddress(_glfw.win32.user32.instance, "GetDpiForWindow");
    _glfw.win32.user32.AdjustWindowRectExForDpi_ = cast(PFN_AdjustWindowRectExForDpi)
        GetProcAddress(_glfw.win32.user32.instance, "AdjustWindowRectExForDpi");

    _glfw.win32.dinput8.instance = LoadLibraryA("dinput8.dll");
    if (_glfw.win32.dinput8.instance)
    {
        _glfw.win32.dinput8.Create = cast(PFN_DirectInput8Create)
...

    {
        ...
        for (i = 0;  names[i];  i++)
        {
            ...
            {
                _glfw.win32.xinput.GetCapabilities = cast(PFN_XInputGetCapabilities)
    ...
    {
        _glfw.win32.dwmapi.IsCompositionEnabled = cast(PFN_DwmIsCompositionEnabled)
            GetProcAddress(_glfw.win32.dwmapi.instance, "DwmIsCompositionEnabled");
        _glfw.win32.dwmapi.Flush = cast(PFN_DwmFlush)
            GetProcAddress(_glfw.win32.dwmapi.instance, "DwmFlush");
        _glfw.win32.dwmapi.EnableBlurBehindWindow = cast(PFN_DwmEnableBlurBehindWindow)
    ...
    {
        _glfw.win32.shcore.SetProcessDpiAwareness_ = cast(PFN_SetProcessDpiAwareness)
            GetProcAddress(_glfw.win32.shcore.instance, "SetProcessDpiAwareness");
        _glfw.win32.shcore.GetDpiForMonitor_ = cast(PFN_GetDpiForMonitor)
    ...
    {
        _glfw.win32.ntdll.RtlVerifyVersionInfo_ = cast(PFN_RtlVerifyVersionInfo)
...
private GLFWbool createHelperWindow() {
    ...
    // Register for HID device notifications
    {
        ...
        memset(&dbi, 0, typeof(dbi).sizeof); //ZeroMemory(&dbi, typeof(dbi).sizeof);
        dbi.dbcc_size = typeof(dbi).sizeof;
        dbi.dbcc_devicetype = DBT_DEVTYP_DEVICEINTERFACE;
        dbi.dbcc_classguid = GUID_DEVINTERFACE_HID;

        _glfw.win32.deviceNotificationHandle =
            RegisterDeviceNotificationW(_glfw.win32.helperWindowHandle,
                                        cast(DEV_BROADCAST_HDR*) &dbi,
...
WCHAR* _glfwCreateWideStringFromUTF8Win32(const(char)* source) {
    ...
    target = cast(WCHAR*) calloc(count, WCHAR.sizeof);
...
char* _glfwCreateUTF8FromWideStringWin32(const(WCHAR)* source) {
    ...
    target = cast(char*) calloc(size, 1);
...
void _glfwInputErrorWin32(int error, const(char)* description) {
    WCHAR[_GLFW_MESSAGE_SIZE] buffer = ""w;
    char[_GLFW_MESSAGE_SIZE] message = "";
    ...
    FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM |
                   ...
                   buffer.ptr,
                   buffer.sizeof / WCHAR.sizeof,
                   null);
    WideCharToMultiByte(CP_UTF8, 0, buffer.ptr, -1, message.ptr, typeof(message).sizeof, null, null);

    _glfwInputError(error, "%s: %s", description, message.ptr);
}
...
void _glfwUpdateKeyNamesWin32() {
    ...
    memset(_glfw.win32.keynames.ptr, 0, typeof(_glfw.win32.keynames).sizeof);

    for (key = GLFW_KEY_SPACE;  key <= GLFW_KEY_LAST;  key++)
    {
        UINT vk;
        int scancode;int length;
        WCHAR[16] chars;
        ...
        length = ToUnicode(vk, scancode, state.ptr,
                           chars.ptr, chars.sizeof / WCHAR.sizeof,
                           0);
        ...
        WideCharToMultiByte(CP_UTF8, 0, chars.ptr, 1,
                            _glfw.win32.keynames[key].ptr,
                            typeof(_glfw.win32.keynames[key]).sizeof,
                            null, null);
    }
}
...
BOOL _glfwIsWindowsVersionOrGreaterWin32(WORD major, WORD minor, WORD sp) {
    OSVERSIONINFOEXW osvi = OSVERSIONINFOEXW(OSVERSIONINFOEXW.sizeof, major, minor, 0, 0, (wchar[128]).init, sp);
...
BOOL _glfwIsWindows10BuildOrGreaterWin32(WORD build) {
    OSVERSIONINFOEXW osvi = OSVERSIONINFOEXW(OSVERSIONINFOEXW.sizeof, 10, 0, build);
...
int _glfwPlatformInit() {
    ...
    SystemParametersInfoW(SPI_SETFOREGROUNDLOCKTIMEOUT, 0, null /*UIntToPtr(0)*/,
                          SPIF_SENDCHANGE);
    ...
    if (_glfwIsWindows10CreatorsUpdateOrGreaterWin32())
        ...
        _glfw.win32.shcore.SetProcessDpiAwareness_(PROCESS_DPI_AWARENESS.PROCESS_PER_MONITOR_DPI_AWARE);
...
void _glfwPlatformTerminate() {
    ...
    SystemParametersInfoW(SPI_SETFOREGROUNDLOCKTIMEOUT, 0,
                          cast(void*) cast(UINT_PTR) _glfw.win32.foregroundLockTimeout,
                          SPIF_SENDCHANGE);
...
const(char)* _glfwPlatformGetVersionString() {
    ...
    version(_GLFW_USE_HYBRID_HPG) {
        enum optimusPart = " hybrid-GPU";
    } else version(_GLFW_USE_OPTIMUS_HPG) {
        enum optimusPart = " hybrid-GPU";
    } else {
        enum optimusPart = "";
    }
    ...
    return _GLFW_VERSION_NUMBER ~ " Win32 WGL EGL OSMesa" ~ clibPart ~ optimusPart ~ dllPart;
}
